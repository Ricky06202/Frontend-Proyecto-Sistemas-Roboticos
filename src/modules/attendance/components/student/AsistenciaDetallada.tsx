		...
		<section className='flex flex-col gap-4 overflow-x-hidden'>
			...
			<div className='flex flex-col gap-4'>
				{materias.map((materia) => (
					<div className='flex flex-col p-4 border-2 rounded-lg overflow-x-auto '>
						<table className=''>
							<thead>
								<tr>
								...
								{materia.dias.map((dia) => (
									<tr className='border-t'>
										...
										<td className='text-center border-l'>
											{dia.nombre} <div>{`${dia.inicio.slice(0, -3)} - ${dia.fin.slice(0, -3)}`}</div>
										</td>
										<td className='text-center border-l'>
											{dia.asistencia.fecha} <div>{dia.asistencia.hora.slice(0, -3)}</div>
										</td>
